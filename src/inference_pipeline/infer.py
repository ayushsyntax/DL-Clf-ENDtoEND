...
class InferencePipeline:
    ...
    def __init__(self, override_model_path: str = None):
        ...
        """
        self.model_path = override_model_path or str(settings.ARTIFACTS_DIR / "best_model.keras")
    ...
    def preprocess_image(self, raw_image_bytes: bytes) -> tf.Tensor:
        ...
        resized_image = tf.image.resize(decoded_image, settings.IMG_SIZE)
    ...
    def predict(self, raw_image_bytes: bytes) -> dict:
        ...
        predicted_label = "Tumor" if confidence_score > 0.5 else "No Tumor"

        return {
            "label": predicted_label,
            "probability": confidence_score,
            "class_idx": 1 if confidence_score > 0.5 else 0
        }
